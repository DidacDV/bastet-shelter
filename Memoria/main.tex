...
En aquest projecte, el nucli de tot el sistema és el backend, ja que és l'encarregat de gestionar tota la lògica i facilitar les comunicacions i interaccions entre el portal web i l'aplicació mòbil. Per aquesta raó, s'ha optat per utilitzar Python amb el framework FastAPI. Python és un llenguatge àmpliament fet servir en el desenvolupament backend, destacat per la seva llegibilitat, la seva gran comunitat i el seu extens ecosistema de llibreries. Aquesta combinació permet realitzar un desenvolupament ràpid i eficient, mantenint l'escalabilitat a futur gràcies al suport natiu de programació asíncrona mitjançant \textit{asyncio} \cite{noauthor_asyncio_nodate}.
...
Finalment, cal destacar l'entorn de desenvolupament: Android Studio, que ofereix eines de debug, un emulador integrat i disseny visual de les interfícies, agilitzant el cicle de desenvolupament en gran manera.
...
El portal web es troba publicat a través de Netlify, que ofereix un servei d'allotjament gratuït suficient per a aplicacions webs, tan estàtiques com a dinàmiques. El desplegament del portal web no té una configuració complexa, ja que es tracta d'una aplicació estàtica que utilitza el mateix backend a través de l'API. El procés de desplegament està automatitzat a través dels mateixos mecanismes que ofereix Netlify, que quan detecta canvis en una branca en concret, en aquest cas la branca \texttt{main}, inicia un procés de deploy que compila l'aplicació i la publica a un URL públic. Aquest URL es pot configurar per a que sigui personalitzada, però en aquest cas s'ha optat per mantenir l'URL genèric que Netlify proporciona, ja que el projecte es troba en una fase inicial i no es preveu un ús massiu que requereixi un URL més professional.

Un afegit de Netlify és el bot que s'integra automàticament amb el repositori de Github, que comprova que el desplegament es realitzi correctament i et permet veure una preview en realitzar una pull request a la branca principal.
...
\subsubsection{Portal web}
Per al portal web, s'ha escollit React principalment per l'experiència prèvia de l'autor amb aquesta llibreria, cosa que ha permès un desenvolupament àgil i eficient dins del temps limitat del projecte. Així i tot, les principals alternatives haurien estat Vue i Angular. Vue és un framework més lleuger i amb una corba d'aprenentatge suau, però amb un ecosistema més petit. Angular, en canvi, és molt complet i robust, però la seva complexitat i la seva corba d'aprenentatge pronunciada no s'ajusten a les necessitats d'un projecte amb els marcs temporals donats. React ofereix un bon equilibri entre flexibilitat, ecosistema i rendiment, i el fet de tenir experiència prèvia amb aquesta tecnologia reforça la decisió.
...
Pel que fa a les alternatives per a la base de dades, la decisió estava en si tenia sentit utilitzar una base de dades relacional o una no relacional (com MongoDB o Cassandra). Com es pot veure al diagrama conceptual del sistema, gairebé totes les entitats formen part de relacions clares entre elles, definint una necessitat de tenir les dades de manera estructurada. A més, d'acord amb els requisits del sistema, no es preveu la necessitat de gestionar una quantitat massiva de dades en la que es pogués aprofitar les particions d'una base de dades no relacional, ni tampoc la necessitat de treballar amb dades semiestructurades.
Una altra part a favor és la consistència, ja que les dades que es generen al propi sistema poden ser utilitzades en una auditoria o per a generar informes, fet que reforça la necessitat de garantir la integritat i la traçabilitat de les dades, aspectes en què els sistemes relacionals destaquen.
Finalment, gràcies a la simplicitat de SQLAlchemy i la seva integració amb FastAPI, es va optar per PostgreSQL com a sistema gestor, consolidant l'opció relacional com la més adequada per a aquest projecte.

Dins de les opcions de bases de dades relacionals, s'ha escollitPostgreSQL per la seva robustesa i escalabilitat ja demostrada i aplicada en molts projectes de producció. També permet gestionar dades geogràfiques a través de l'extensió PostGIS \cite{todo:postgis}, que pot ser útil a futur per a totes aquelles funcionalitats relacionades amb la localització de la protectora, els refugis i els adoptants, com ara la visualització de mapes de protectores o la cerca d'animals disponibles per a l'adopció a prop de la ubicació de l'adoptant.

\subsubsection{Emmagatzematge d'arxius}
En aquest servei, s'ha escollit Cloudinary a causa de la facilitat d'ús i la configuració ràpida que ofereix. En un principi, s'havia considerat únicament per guardar les imatges del sistema (tant dels animals com dels anuncis), però amb el requisit de poder generar i emmagatzemar els contractes d'adopció en format PDF, es va veure que també podia ser una solució eficient per a aquest tipus d'arxius, evitant haver de realitzar la inicialització i configuració d'un altre servei diferent per a aquest mateix propòsit.
Tot i aixó, s'han considerat com a alternatives Amazon S3 i Firebase Storage. Ambdues són solucions molt escalables i fiables, però pateixen de requerir una dedicació més elevada per començar a utilitzar-los. Per exemple, Amazon S3 requereix configurar un bucket, establir polítiques d'accés i generar credencials, mentre que Firebase Storage necessita de configurar un projecte a Firebase, habilitar el servei d'emmagatzematge i també establir regles d'accés. Gràcies a la simplicitat de Cloudinary, es pot integrar el servei en el backend en molt poc temps, cosa que permet centrar-se en altres aspectes del projecte més crítics segons la planificació.
\newpage
