...
class TestShiftParticipantRepository:
    repo = ShiftParticipantRepository(None)
    ...
    def test_get_by_volunteer(self, db):
        shelter = _create_shelter(db)
        refuge = Refuge(name="Test Refuge", province_id="08", shelter_id=shelter.id)
        db.add(refuge)
        db.commit()
        ...
        result = self.repo.get_by_volunteer(db, volunteer.id)
        assert len(result) == 1
