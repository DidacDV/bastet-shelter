...
A més, s'utilitzen altres llibreries complementàries: APScheduler \cite{TODO:citeapscheduler} per a la planificació de tasques periòdiques en segon pla, FastAPI-Mail \cite{TODO:citefastapi-mail} per a l'enviament de correus electrònics i SQLAdmin \cite{TODO:citesqladmin} per a la generació del panell d'administració de la base de dades.

\subsubsection{Aplicació mòbil}
Per dur a terme el desenvolupament de l'aplicació mòbil, s'ha escollit el llenguatge dart amb el framework Flutter. Dart és un llenguatge modern, tipat i orientat a objectes, desenvolupat per Google, i ofereix un rendiment natiu en qualsevol plataforma i accelera el treball gràcies a la funció \textit{hot reload} \cite{TODO:hotreload}. Flutter és un framework per crear interfícies d'usuari multiplataforma a través d'un únic codi base, asoolint un rendiment gairebé natiu gràcies a Dart \cite{TODO:fluttervsnative} i a la seva arquitectura basada en widgets \cite{todo:citearchitectureflutter}. A més, Flutter disposa de moltes eines oficials i de tercers que simplifica la implementació de funcionalitats complexes, com ara la gestió de l'estat, la internacionalització o la generació de PDFs. Una altra raó per la qual s'ha escollit Flutter és l'entorn en el que et permet treballar, ja que s'integra sense problema amb Android Studio, i agilitzen molt el desenvolupament gràcies a les seves eines de debug i disseny visual.
...
Pel que fa a les llibreries utilitzades, Riverpod \cite{TODO:citeriverpod} és la solució escollida per a la gestió de l'estat de l'aplicació. S'han definit proveïdors per a les dades que es comparteixen entre pantalles, tant per a consultes generals com específiques, com ara l'obtenció de tots els torns o els torns d'una setmana i refugi concrets. D'aquesta manera, evitem trucades innecessàries al servidor, reduint la seva càrrega de treball i el temps d'espera de l'usuari. GetIt \cite{TODO:citegetit} s'utilitza com a contenidor de dependències, tot i que el patró que aplica s'explica amb més detall a la secció de disseny. Per altra banda, \texttt{shared\_preferences} \cite{TODO:citeshared_preferences} permet persistir dades locals al dispositiu, com ara el token d'autenticació de l'usuari.
...
Per poder provar l'enviament de correus electrònics, s'ha utilitzat \textit{Mailtrap} \cite{TODO:citemailtrap}, un servei que actua com a un entorn de correu virtual, permetent capturar i visualitzar els correus que s'enviarien a un servidor de correu real. Això és especialment útil durant les proves, ja que permet verificar el contingut dels correus generats. En producció, s'utilitzaria un servidor SMTP real.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{MailTrapSchema.png}
    \caption[Visualització del Mailtrap]{Visualització del Mailtrap. Font: \cite{todo:citeAutomatizacióndepruebasdeEMAILconMAILTRAP}}
    \label{fig:mailtrap_illustration}
\end{figure}

Ja que el sistema està inicialment pensat per a un àmbit nacional, la localització es basa en les províncies d'Espanya. Per a obtenir aquesta informació i mantenir-la actualitzada, s'ha integrat el servei de \textit{GeoAPI.es} \cite{TODO:citegeoapi}, que proporciona dades geogràfiques oficials de les províncies espanyoles a través d'una API. S'ha configurat una tasca periòdica que consulta aquestes dades per mantenir-les sincronitzades.
